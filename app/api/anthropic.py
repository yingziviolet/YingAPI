...
async def messages(
    ...
    # 精确缓存(翻译后的 OpenAI 请求体做 key,与 OpenAI 入口共享缓存);流式暂不查缓存
    ...
    if cache_key:
        ...
        if cached is not None:
            ...
            metrics.record_request(
                model=model, channel="(cache)", status="cache_hit", cache="hit",
    ...
    if not stream:
        try:
            data, channel = await forwarder.forward_non_stream(
                client, channels, model, openai_body, settings, fernet, breaker=app.state.breaker
            )
            ...
            return anthropic_error(exc.status_code, exc.message, err_type="api_error")
